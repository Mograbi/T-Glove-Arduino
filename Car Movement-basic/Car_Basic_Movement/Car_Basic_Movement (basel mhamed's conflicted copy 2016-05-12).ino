...
Command parsing_command(String input) {
	...
	String param1;
	if (input.indexOf(",") != -1) {
		String int1 = input.substring(1, input.indexOf(",", 0) - 1);
		angle = atoi(int1.c_str());
		String int2 = input.substring(input.indexOf(",", 0) + 1,
			input.indexOf(">", 0) - input.indexOf(",", 0) - 1);
		speed = atoi(int2.c_str());
		mode = (MOVEMENT);
	}
	else {
		param1 = input.substring(1, input.indexOf(">", 0) - 1);
		mode = PAUSE;
	}
